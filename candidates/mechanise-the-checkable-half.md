---
name: mechanise-the-checkable-half
...
status: candidate
evidence:
  - "The doc gates check paths, constants, landed rows, owned definitions, catalogue rows and heading anchors — all resolvable — while three named judgement classes stay with an agent evaluator"
  - "job #548 — a deletion job's accounting mechanised, its judgement left to two agents and a human"
rationale: >
...
related: [reviewers-read-they-do-not-run, human-approval-only-where-no-gate-can-judge, every-gate-has-a-test-suite]
---
...
The split is visible in the two doc evaluators: a shell gate resolves six
mechanical claim classes whole-tree in under a second, while an agent evaluator
owns exactly three classes no script can decide. Neither duplicates the other,
and each rejection cites which side it came from.
